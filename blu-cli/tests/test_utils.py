...
from blu.utils.config import ensure_pypirc
...
@patch("blu.utils.directories.os.path.exists")
@patch("blu.utils.directories.create_directory")
@patch("blu.utils.directories.get_subdirectories")
@patch("blu.utils.directories.questionary")
def test_detect_directory_developer_exists(
    ...
    mock_questionary.confirm.return_value.ask.return_value = (
        True  # Simulate user saying yes
    )
    mock_get_subdirectories.return_value = (
        {}
    )  # No subdirectories in the Developer folder
...
def test_detect_directory_create_developer(
    ...
    mock_get_subdirectories.return_value = (
        {}
    )  # No subdirectories in the fallback folder
...
def test_detect_directory_create_new_directory(
    ...
    mock_get_subdirectories.side_effect = lambda path: {
        "sub1": os.path.join(path, "sub1")
    }
    mock_questionary.confirm.return_value.ask.return_value = (
        False  # Simulate user saying no to creating Developer
    )
    mock_questionary.select.return_value.ask.side_effect = [
        "Create a New Directory"
    ]  # User selects create new dir
    mock_questionary.path.return_value.ask.return_value = (
        "/new/path"  # New path entered by user
    )
...
@patch("blu.utils.run_command.subprocess.run")
@patch("blu.utils.run_command.Halo")
...
@patch("blu.utils.config.questionary")
def test_ensure_pypirc_creates_new_file(
    ...
    mock_questionary.confirm.return_value.ask.return_value = (
        True  # User agrees to create the file
    )
...
"""
    ...
    mock_questionary.confirm.return_value.ask.return_value = (
        True  # User agrees to update the file
    )
    mock_questionary.text.return_value.ask.side_effect = ["__token__"]  # Mock username
    mock_questionary.password.return_value.ask.side_effect = [
        "test_token"
...
def test_ensure_pypirc_aborts_if_user_declines_creation(
    ...
    monkeypatch.setattr("os.path.expanduser", lambda path: str(mock_pypirc_file))
    mock_questionary.confirm.return_value.ask.return_value = (
        False  # User declines to create the file
    )
