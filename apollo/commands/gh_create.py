...
def gh_create():
    """Main logic for creating a GitHub repository."""
    ...
    if parent_dir not in config["cached_directories"]:
        save_dir = questionary.confirm(f"Would you like to save '{parent_dir}' for future use?").ask()
        if save_dir:
    ...
    try:
        # spinner.start(f"Creating {visibility} repository on GitHub using GitHub CLI...")
        run_command(f"gh repo create {repo_name} --{visibility} --description '{repo_description}' --confirm", start=f"Creating {visibility} repository on GitHub using GitHub CLI...")
        spinner.succeed(f"Created {visibility} repository on GitHub using GitHub CLI Successfully!")
    except subprocess.CalledProcessError as e:
        spinner.fail(f"Creating {visibility} repository on GitHub using GitHub CLI failed with error: {e.stderr}")
    ...
    # spinner.succeed("Initialized Git repository Successfully!")
    
    # spinner.start("Staging README.md...")
    run_command("git add README.md", cwd=full_path, start="Staging README.md...")
    # spinner.succeed("Staged README.md Successfully!")
    
    # print('Committing with message "initializing repo with README"...')
    run_command('git commit -m "initializing repo with README"', cwd=full_path, start="Committing with message 'initializing repo with README'...")
    # print("Renaming default branch to 'main'...")
    run_command("git branch -M main", cwd=full_path, start="Renaming default branch to 'main'...")
    # print(f"Adding remote origin: {git_url}...")
    run_command(f"git remote add origin {git_url}", cwd=full_path, start=f"Adding remote origin: {git_url}...")
    # print("Pushing to remote repository...")
    run_command("git push -u origin main", cwd=full_path, start="Pushing to remote repository...")
